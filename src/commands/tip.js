...
function tip (message, Zcash, amount, toAccount) {
  ...
  if (!toAccount) {
    message.reply(message.author.toString() + ' ,' + NEED_ADDRESS_TEXT)
    return
  }
  ...
  db.get(toAccount, function(err, address) {
    ...
    if (haddress  == '' + 'undefined') {
      message.channel.send(message.author.toString() + ', User specified not found.')
      return;
    }
          ...
          var amount = data.result - txFee;
            ...
            post('z_sendmany', [gaddress, [{ address: haddress, amount: amountInt }, { address: gaddress, amount: bal }]], function(err, data) {
                if (err != null) {
                    ...
                    message.channel.send(message.author.toString() + ', Sent **' +  amountInt + '** Koto to ``' +  haddress + '``')
            ...
            message.channel.send(message.author.toString() + 'Your tipping amount is below the threshold of **0.1** Koto. Please raise the amount.');
              }
            }
        else { 
           console.log ('Not enough balance')
           message.channel.send(message.author.toString() + ', You do not have enough balance/ Your tipping amount is below the threshold of **0.1** Koto. Please raise the amount.');
        }
        });
      })
    })
